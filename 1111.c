...
int carSpeed   = 220;   // سرعة التتبّع (كانت 190)
int turnSpeed  = 230;   // سرعة الدوران (كانت 200)
...
bool anyObstacle(){
  return irActive(IR_R) || irActive(IR_F) || irActive(IR_L);
}

/* ===== تفادي سريع بالـ ir (ثلاث حساسات) ===== */
void avoidByIR(){
  bool r  = irActive(IR_R);
  bool f  = irActive(IR_F);
...
bool acquire(){
  ...
  if(bestA==rightAngle){ turnRight(); delay(180); stopAll(); }
  else if(bestA==leftAngle){ turnLeft(); delay(180); stopAll(); }

  lastSeenMs = millis();
  return true;
}

/* ===== follow using front only (أسرع نبض + توقف أقل) ===== */
void follow(){
  long d = distCM();
  ...
  if(d > maxFollow){
    forward();  delay(170);   // كانت 140
    stopAll();  delay(10);    // كانت 20
  } else if(d < minFollow && d>backLimit){
    back();     delay(100);
    stopAll();  delay(10);
  } else {
    // داخل منطقة الراحة: تحريك خفيف بدل الوقوف الطويل
    forward();  delay(90);
    stopAll();  delay(8);
  }
}
...
void loop(){
  ...
  if(mode==ACQUIRE){
    ...
    else { turnRight(); delay(130); stopAll(); delay(30); }
    return;
  }
  follow();
}
